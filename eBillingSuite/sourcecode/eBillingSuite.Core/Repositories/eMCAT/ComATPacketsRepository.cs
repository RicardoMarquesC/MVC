...
namespace eBillingSuite.Repositories
{
    public class ComATPacketRepository : IComATPacketsRepository
    {
        ...
        public List<ComATPackets> GetComATPackets(string instances)
        {
            List<ComATPackets> finalList;
            ...
            try
            {
                using (EBC_DB entidadeEBC = new EBC_DB())
                {
                    if (instances == "*")
                        ...
                        finalList = entidadeEBC.ComATPackets.Where(o => listInstances.Contains(o.NIFEmissor)).OrderByDescending(x => x.LastSentDate).ToList();
                }
            }
            catch (Exception e)
            {
