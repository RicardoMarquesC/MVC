...
namespace eBillingSuite.ViewModels
{
    public class EBCDigitalCertificateVM
    {
            ...
            IeBillingSuiteRequestContext _context)
        {
            ...
            this.SerialNumber = model== null ? "" : model[0].KeyValue;

            //Instances
            var values = eCConfigRepositories.instancesRepository.GetEBC_Instances(_context.UserIdentity.Instances);
            AvailableInstances = values
            .Select(v => new SelectListItem
            {
                Text = v.Name,
                Value = v.PKID.ToString(),
                Selected = v.PKID == model[0].FKInstanceID
            })
            .ToList();
        ...
        protected string GetConfigName(string KeyName)
        {
            ...
            else if (KeyName.Equals("WS_Url"))
                return "URL";
            else if (KeyName.Equals("WS_Username"))
                return "Utilizador";
            else if (KeyName.Equals("WS_Password"))
                return "Palavra-chave";
            else if (KeyName.Equals("WS_Domain"))
                return "Dominio";
            else
                return null;
